...
var	GEO_FIELD_MIN 		= 0,
	GEO_FIELD_MAX 		= 1,
	GEO_FIELD_COUNTRY 	= 2,
	GEO_FIELD_OPERATOR 	= 3,
	GEO_DIR_DATA 		= __dirname + "/../data";

// Load and parse geo base
function _powerup() {
	var fs = require('fs');
	var cidr = fs.readFileSync(GEO_DIR_DATA + "/operator.txt");
...
function _normalize(row) {
	return {
		...
		"operator":  row[GEO_FIELD_OPERATOR]
	};
}
...
module.exports.lookup = function(ip) {
	...
	while (low <= high) {
		...
		if (gindex[i][GEO_FIELD_MIN] < find) { if(gindex[i][GEO_FIELD_MAX] > find)break; low  = i + 1; continue; }
		break;
	}                        
	var row = gindex[i];
